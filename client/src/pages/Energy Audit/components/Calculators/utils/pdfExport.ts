...
interface JsPDFWithAutoTable extends jsPDF {
  ...
  autoTable: (options: AutoTableOptions) => any;
}
...
export const exportLPDToPDF = (data: LPDExportData): string => {
  ...
  // Energy Savings Section (if available)
  if (results.potentialSavings) {
    const savingsY = recsY + 8 + (results.recommendations.length * 6) + 10;
    doc.setFontSize(14);
    doc.text('Energy Savings Potential', 14, savingsY);
    
    doc.setFontSize(10);
    const { wattageSavings, percentageSavings, estimatedAnnualKwh, estimatedAnnualCost, paybackPeriod } = results.potentialSavings;
    
    doc.text(`Power Savings: ${formatNumber(wattageSavings)} W (${formatNumber(percentageSavings)}% reduction)`, 14, savingsY + 8);
    doc.text(`Annual Energy Savings: ${formatNumber(estimatedAnnualKwh)} kWh/year`, 14, savingsY + 14);
    doc.text(`Annual Cost Savings: $${formatNumber(estimatedAnnualCost)}/year`, 14, savingsY + 20);
    
    if (paybackPeriod) {
      doc.text(`Estimated Payback Period: ${formatNumber(paybackPeriod)} years`, 14, savingsY + 26);
    }
  }
